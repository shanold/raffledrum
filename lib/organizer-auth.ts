...
export async function organizerLockEnabled() {
  return (await runtimeValue("LOCK_ENABLED")).trim().toLowerCase() !== "false";
}
...
export async function organizerAuthConfigured() {
  if (!(await organizerLockEnabled())) return true;
  return Boolean(
    (await runtimeValue("ORGANIZER_PASSWORD_HASH")) &&
    (await runtimeValue("SESSION_SECRET")),
  );
}
...
export async function verifyOrganizerPassword(password: string) {
  ...
  return (
    expected.length === 64 && safeEqual(await sha256Hex(password), expected)
  );
}

export async function createOrganizerSession(request: Request) {
  const secret = await runtimeValue("SESSION_SECRET"),
    expires = Math.floor(Date.now() / 1000) + SESSION_SECONDS,
    payload = `organizer:${expires}`,
    signature = await hmacHex(payload, secret),
...
export async function requireOrganizer(request: Request) {
  ...
  return Response.json(
    { error: "Organizer sign-in required." },
